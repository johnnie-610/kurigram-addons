...
__all__ = ['OnUserStatus']


class OnUserStatus:
    """A class providing decorators for handling on user status in Pyrogram.

    This class provides the `on_user_status` decorator which can be used to
    register on user status handlers in a Pyrogram application.
    """
    ...
    def on_user_status(
        ...
        """Decorator for handling on user status.

    This does the same thing as :meth:`~pyrogram.Client.add_handler` using the
    :obj:`~pyrogram.handlers.UserStatusHandler`.

    The decorated function should have the following signature:
        async def callback(client: Client, user: User) -> Any:

    Parameters:
        filters: One or more filters to allow only a subset of on user status
    ...
    """
        def decorator(func: T) -> T:
            ...
            @wraps(func)
            async def wrapper(client: 'pyrogram.Client', user: 'pyrogram.types.User') -> Any:
                return await func(client, user)

            handler = UserStatusHandler(wrapper, filters)
