...
class OnPoll:
    """A class providing decorators for handling on poll in Pyrogram.

    This class provides the `on_poll` decorator which can be used to
    register on poll handlers in a Pyrogram application.
    """
    ...
    def on_poll(
        ...
        """Decorator for handling on poll.

    This does the same thing as :meth:`~pyrogram.Client.add_handler` using the
    :obj:`~pyrogram.handlers.PollHandler`.

    The decorated function should have the following signature:
        async def callback(client: Client, poll: Poll) -> Any:

    Parameters:
        filters: One or more filters to allow only a subset of on poll
    ...
    """
        def decorator(func: T) -> T:
            ...
            @wraps(func)
            async def wrapper(client: 'pyrogram.Client', poll: 'pyrogram.types.Poll') -> Any:
                return await func(client, poll)

            handler = PollHandler(wrapper, filters)
