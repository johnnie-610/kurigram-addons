...
__all__ = ['OnDisconnect']


class OnDisconnect:
    """A class providing decorators for handling on disconnect in Pyrogram.

    This class provides the `on_disconnect` decorator which can be used to
    register on disconnect handlers in a Pyrogram application.
    """
    ...
    def on_disconnect(
        ...
        """Decorator for handling on disconnect.

    This does the same thing as :meth:`~pyrogram.Client.add_handler` using the
    :obj:`~pyrogram.handlers.DisconnectHandler`.

    The decorated function should have the following signature:
        async def callback(client: Client, disconnect: Disconnect) -> Any:

    Parameters:
        filters: One or more filters to allow only a subset of on disconnect
    ...
    """
        def decorator(func: T) -> T:
            ...
            @wraps(func)
            async def wrapper(client: 'pyrogram.Client', disconnect: 'pyrogram.types.Disconnect') -> Any:
                return await func(client, disconnect)

            handler = DisconnectHandler(wrapper, filters)
