...
__all__ = ['OnChatMemberUpdated']


class OnChatMemberUpdated:
    """A class providing decorators for handling on chat member updated in Pyrogram.

    This class provides the `on_chat_member_updated` decorator which can be used to
    register on chat member updated handlers in a Pyrogram application.
    """
    ...
    def on_chat_member_updated(
        ...
        """Decorator for handling on chat member updated.

    This does the same thing as :meth:`~pyrogram.Client.add_handler` using the
    :obj:`~pyrogram.handlers.ChatMemberUpdatedHandler`.

    The decorated function should have the following signature:
        async def callback(client: Client, chat_member_updated: ChatMemberUpdated) -> Any:

    Parameters:
        filters: One or more filters to allow only a subset of on chat member updated
    ...
    """
        def decorator(func: T) -> T:
            ...
            @wraps(func)
            async def wrapper(client: 'pyrogram.Client', chat_member_updated: 'pyrogram.types.ChatMemberUpdated') -> Any:
                return await func(client, chat_member_updated)

            handler = ChatMemberUpdatedHandler(wrapper, filters)
