...
class FSMContext:
    ...
    async def set_state(self, state: str, *, ttl: Optional[int] = None) -> None:
        """Set the FSM state."""
        state_str = str(state)
        current_state = await self._storage.get_state(self._id) or {"name": None, "data": {}}
        await self._storage.set_state(self._id, {"name": state_str, "data": current_state["data"]}, ttl=ttl)
    ...
    async def update_data(self, **kwargs: Any) -> Dict[str, Any]:
        ...
        state: dict = await self._storage.get_state(self._id) or {"name": None, "data": {}}
        dt: dict = state["data"]
        dt.update(kwargs)
        print(f"[DEBUG] Updated FSM data: {state}")
