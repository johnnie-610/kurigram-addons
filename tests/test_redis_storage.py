...
async def test_compare_and_set_and_basic_ops():
    ...
    ok = await store.compare_and_set("user:1", {"state":"A"}, expected_state=None)
    assert ok is True
    state = await store.get_state("user:1")
    assert state["state"] == "A"

    ok2 = await store.compare_and_set("user:1", {"state":"B"}, expected_state={"state":"A"})
    assert ok2 is True
    # wrong expected
    ok3 = await store.compare_and_set("user:1", {"state":"C"}, expected_state={"state":"X"})
    assert ok3 is False
